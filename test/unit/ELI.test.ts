...
describe(`[${CODE}] rules`, () => {
  ...
  describe('source parsing', () => {
    it('parses text without src', () => {
      const result = extractSourceEnhanced('1. You Able to Do ‘Running Race With Horses’? (10 min.)', CODE);

      expect(result.fulltitle).toBe('1. You Able to Do ‘Running Race With Horses’?');
      expect(result.type).toBe('You Able to Do ‘Running Race With Horses’?');
    ...
    it('parses text with src', () => {
      const result = extractSourceEnhanced(
        '6. Talk (5 min.) lmd appendix A point 17—Title: Jesus Was Good Teacher and His Advice Can Always Work. (th study 14)',
        CODE,
      );
      ...
      expect(result.src).toBe(
        'lmd appendix A point 17—Title: Jesus Was Good Teacher and His Advice Can Always Work. (th study 14)',
      );
