...
describe(`[${CODE}] rules`, () => {
  ...
  describe('source parsing', () => {
    it('parses text without src', () => {
      const result = extractSourceEnhanced('1. Akôry ‘Hahavitanao Han̈ano Lakorsy amy Sevaly?’ (10 min.)', CODE);

      expect(result.fulltitle).toBe('1. Akôry ‘Hahavitanao Han̈ano Lakorsy amy Sevaly?’');
      expect(result.type).toBe('Akôry ‘Hahavitanao Han̈ano Lakorsy amy Sevaly?’');
    ...
    it('parses text with src', () => {
      const result = extractSourceEnhanced(
        '5. Mitsidiky (4 min.) ISAKA TRAN̈O. Olo io manan-janaka. (lmd leson 3: Hevitry 3)',
        CODE,
      );

      expect(result.fulltitle).toBe('5. Mitsidiky');
      expect(result.type).toBe('Mitsidiky');
      expect(result.src).toBe('ISAKA TRAN̈O. Olo io manan-janaka. (lmd leson 3: Hevitry 3)');
  ...
  describe('MWB date parsing', () => {
    // Pattern 1: range
    it.each([
      ['5-11 Zanvie', 2026, '2026/01/05'],
      ['2-8 Fevrie', 2026, '2026/02/02'],
      ['2-8 Marsy', 2026, '2026/03/02'],
      ['6-12 Avrily', 2026, '2026/04/06'],
      ['4-10 May', 2026, '2026/05/04'],
      ['1-7 Ziain', 2026, '2026/06/01'],
      ['6-12 Zie', 2026, '2026/07/06'],
      ['3-9 Aoty', 2026, '2026/08/03'],
