...
describe(`[${CODE}] rules`, () => {
  ...
  describe('source parsing', () => {
    it('parses text without src', () => {
      const result = extractSourceEnhanced('1. 忠貞地對待耶和華（10 分鐘）', CODE);

      expect(result.fulltitle).toBe('1. 忠貞地對待耶和華');
      expect(result.type).toBe('忠貞地對待耶和華');
      expect(result.src).toBeUndefined();
      expect(result.time).toBe(10);
    });

    it('parses text with src', () => {
      const result = extractSourceEnhanced('4. 初次交談（3 分鐘）運用介紹詞（lmd 第1課重點5）', CODE);

      expect(result.fulltitle).toBe('4. 初次交談');
      expect(result.type).toBe('初次交談');
      expect(result.src).toBe('運用介紹詞（lmd 第1課重點5）');
