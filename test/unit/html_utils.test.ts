...
describe('html_utils', () => {
  ...
  describe('parseMWB', () => {
    ...
    it('rejects a four-digit out-of-range number as a concluding song and falls back', async () => {
      const html = `
        <div>
          <h1>Date</h1>
          <h2>Reading</h2>
      ...
      expect(result[0].mwb_song_conclude).toBe(10);
      expect(result[0].mwb_song_conclude_locale).toBe('10');
    });
  });
});
