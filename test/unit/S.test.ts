...
describe(`[${CODE}] rules`, () => {
  ...
  describe('source parsing', () => {
    ...
    it('parses text with src', () => {
      const result = extractSourceEnhanced(
        '5. Haga revisitas (4 mins.) DE CASA EN CASA. La persona tiene hijos (lmd lección 3 punto 3).',
        CODE,
      );

      expect(result.fulltitle).toBe('5. Haga revisitas');
      expect(result.type).toBe('Haga revisitas');
      expect(result.src).toBe('DE CASA EN CASA. La persona tiene hijos (lmd lección 3 punto 3).');
  ...
  describe('W study date parsing', () => {
    ...
    // Pattern 2: single day
    it('parses single day format', () => {
      const src = '5 de Enero de 2026';
      const result = extractWTStudyDate(src, CODE);
      expect(result).toBe('2026/01/05');
    });

    it('throws an error for unknown month', () => {
      const src = '5 al 11 de Foobar de 2026';
